...
腳本是冪等的，會自動：改寫制度檔內的 HARNESS 路徑（repo 位置變了才改）、
備份既有的 `~/.claude/CLAUDE.md`、建立 symlink、
`~/.claude/env.md` 不存在時從 `claude/env.example.md` 複製範本、跑健康檢查。
...
5. 之後任何 Claude 相關工具安裝／升級後，重跑 `bash claude/check-harness.sh`。
